...
4. `Sending Type`ごとに設定を行います
   - `Media`の場合: `Inputs` の `Audio` および `Video` で送信するデバイスを選択します
   - `Display`の場合: `Input` の `Select` で送信する範囲を選択します
5. `Key` に SkyWay の API Key を入力します
6. `Peer` の `Switch` をクリックして映像送信を開始・停止できます
7. `URL` の `Copy` をクリックして[映像受信側](#映像受信側)のリンクを取得できます
